    ...
    <div className="flex flex-wrap items-center gap-4 py-6 my-6 border-t border-border/40">
      {technologies.map(({ name, Icon }) => (
        <div
          ...
          title={name}
        >
          <Icon className={`${name === "R" || name === "Prettier" || name === "Subscription" ? "w-10 h-10" : name === "React Hook Form" || name === "registry/ui" || name === "Triangle" || name === "Framer" ? "w-11 h-11" : name === "Tailwind CSS" || name === "Prisma" ? "w-13 h-13" : "w-12 h-12"} text-black`} />
        </div>
      ))}
    </div>
  )
