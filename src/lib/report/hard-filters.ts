...
export function runHardFilters(
  ...
  // HF3 — anonymous + captcha CONFIGURED-but-failed → reject. When captchaValid
  // is null (Turnstile not enforced: authenticated, or unconfigured deployment)
  // we do NOT reject — the report proceeds at a degraded trust score. Only an
  // explicit false (token present/required but invalid) is a bot signal.
