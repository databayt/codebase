...
// Authentic Airbnb "Bélo" mark (24×24 viewBox), brand coral #FF5A5F.
const AIRBNB_ICON_PATH =
...
export default function Hero({ dictionary, params }: HeroProps) {
    return (
        <PageHeader
            announcement={
                <Announcement
                    href={`/${params.lang}/arts`}
                    title="Airbnb"
                    label="Airbnb fine arts"
                    iconPath={AIRBNB_ICON_PATH}
                    iconClassName="text-[#FF5A5F]"
                />
            }
            heading={dictionary.iconpage?.heading || "High end arts"}
            ...
            actions={
                <TwoButtons
                    primaryLabel={dictionary.actions?.browseIcons || "Browse arts"}
                    primaryHref={`/${params.lang}/arts`}
                    secondaryLabel={dictionary.actions?.iconDocs || "request arts"}
                    secondaryHref={`/${params.lang}/atoms/report-issue`}
                />
            }
        />
    );
