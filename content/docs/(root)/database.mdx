...
```
prisma/
├── schema.prisma        — Main configuration with datasource and generator
...
│   ├── students.prisma  — Student, Guardian, StudentGuardian
...
│   ├── attendance.prisma — Attendance with status tracking
│   └── auth.prisma      — User, Account, tokens, 2FA
├── generator/           — Database seeding scripts
│   ├── seed.ts          — Simple seeding for basic data
│   ├── multi-seed.ts    — Multi-school seeding orchestrator
│   └── seed-selector.ts — Advanced seed selection options
├── migrations/          — Auto-generated migration files
│   └── migration_lock.toml — Migration provider lock
├── README.md            — Database documentation and guide
└── ISSUE.md             — Known issues and troubleshooting
```
