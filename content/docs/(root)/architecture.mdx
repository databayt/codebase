...
For example, the route `/leads` creates:
- `app/[lang]/leads/` — page.tsx, layout.tsx, loading.tsx
- `components/leads/` — content.tsx, actions.ts, form.tsx, validation.ts, type.ts, hooks, etc.
...
1. **Presentation Layer (Next.js/React)** – Renders the components defined in `form.tsx`, `card.tsx`, and `content.tsx`
...
3. **Data & Persistence Layer (Neon/Prisma)** – Interacts with the database based on the schemas and types defined in `type.ts`
4. **AI & Automation Core (Claude/LangGraph)** – Complex automation agents invoked from within `actions.ts`, acting as the most powerful type of "microservice"
...
3. The serverless function uses the type-safe Prisma client to query Neon, using interfaces from `type.ts`
...
This architecture prioritizes **Developer Experience**, **Maintainability**, **Reusability**, **Type-Safety**, and **Performance** through a serverless-first, Edge-compatible approach.
